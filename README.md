...
starter 不再自动探测数据库类型，必须在配置中显式指定 `mybatis-plus.upsert.db-type`。

支持的值：`mysql`、`postgresql`、`oracle`、`sqlserver`、`h2`、`custom`。

```yaml
# 例：TiDB 兼容 MySQL 语法，手动指定
mybatis-plus:
  upsert:
    db-type: mysql
```
