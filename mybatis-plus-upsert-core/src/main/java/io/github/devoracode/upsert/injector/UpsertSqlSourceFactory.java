 ...
 * once and baked into the {@code SqlSource} (cached in the {@code MappedStatement}).
 * In dynamic-datasource mode (when the dialect is a {@link DynamicUpsertDialect}), a
 * {@link RoutingUpsertSqlSource} is created instead so the actual dialect can be resolved
 * at runtime per data source.
 *
...
final class UpsertSqlSourceFactory {
    ...
    static SqlSource create(Configuration configuration,
        ...
        // Single datasource mode: build SQL once and bake into SqlSource
        String sql = batch
